...
<div class="flex min-h-screen flex-col bg-surface-0">
	<Header backHref="/" backLabel="← Foody" title="Wochenplan" userName={data.user.name}>
		{#snippet actions()}
			<a href="/recipes" class="text-sm text-secondary no-underline hover:text-primary">Rezepte</a
			>
		{/snippet}
	</Header>
	...
	<main class="mx-auto w-full max-w-2xl flex-1 px-6 py-6">
		...
		{#if planned.length > 0}
			<section class="mb-8">
				<h2
					class="m-0 mb-3 flex items-center gap-2 text-[13px] font-semibold tracking-wide text-secondary uppercase"
				>
					Diese Woche
					<span
						class="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-brand px-1.5 text-[11px] font-bold text-white"
					>
						{planned.length}
					</span>
				</h2>
		...
		{#if unplanned.length > 0}
			<section>
				<h2 class="m-0 mb-3 text-[13px] font-semibold tracking-wide text-secondary uppercase">
		...
		{#if filtered.length === 0}
			<div class="py-12 text-center text-sm text-secondary">
				{#if items.length === 0}
